...
export class ExercismClientController {
  ...
  async downloadSolutionFiles(track: Track, exercise: Exercise): Promise<void> {}
  ...
  async downloadExerciseFiles(track: Track, exercise: Exercise): Promise<void> {
    ...
    if (solutionDataResp.status !== 200) {
      throw new Error(solutionDataResp.statusText);
    }
  ...
  async writeMetaDataFile(exerciseDir: string, payload: DownloadPayload): Promise<void> {
    return fs.outputFile(
      ...
      JSON.stringify({
        ...payload.solution.exercise,
        ...payload.solution.user
      })
    );
  }
}
